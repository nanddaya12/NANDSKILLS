<div class="min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden font-sans">
    ...
    <div class="w-full max-w-md p-8 rounded-2xl bg-white border border-slate-200 shadow-xl shadow-slate-200/50 relative z-10 mx-4">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-slate-555 text-sm mt-1">Recover Academy Access</p>
        </div>
        ...
        @if ($statusMessage)
            <div class="mb-5 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm flex flex-col gap-2">
                ...
                @if ($simulationLink)
                    <div class="mt-2 p-2.5 rounded bg-slate-50 border border-emerald-300 text-xs">
                        <span class="text-slate-500 font-bold block mb-1">Local Testing Link:</span>
                        <a href="{{ $simulationLink }}" class="text-blue-600 font-semibold break-all hover:underline">{{ $simulationLink }}</a>
                    </div>
                @endif
            </div>
        @endif
        ...
        <form wire:submit.prevent="sendResetLink" class="space-y-5">
            ...
            <button type="submit" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove>Send Recovery Link</span>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 text-center">
            <p class="text-xs text-slate-500">Remember your password? <a href="{{ route('login') }}" class="text-blue-600 font-bold hover:underline">Back to Login</a></p>
        </div>
    </div>
</div>
